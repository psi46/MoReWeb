...
class TestResult(AbstractClasses.GeneralTestResult.GeneralTestResult):
	def CustomInit(self):
		self.Name='CMSPixel_ModuleTestGroup_Module_Noise_TestResult'
	...
	def SpecialPopulateData(self,TestResultObject, Parameters):
		...
		TestResultObject.ResultData['KeyValueDictPairs'] = {
			'N': {
				'Value':'{:1.0f}'.format(Integral), 
				'Label':'N'
			},
			'mu': {
				'Value':'{:1.2f}'.format(Mean), 
				'Label':'μ'
			},
			'sigma':{
				'Value':'{:1.2f}'.format(RMS), 
				'Label':'σ'
			}
		}
		...
		if under:
			TestResultObject.ResultData['KeyValueDictPairs']['under'] = {'Value':'{:1.2f}'.format(under), 'Label':'<='}
			TestResultObject.ResultData['KeyList'] += 'under'
		if over:
			TestResultObject.ResultData['KeyValueDictPairs']['over'] = {'Value':'{:1.2f}'.format(over), 'Label':'>='}
