...
class TestResult(AbstractClasses.GeneralTestResult.GeneralTestResult):
	def CustomInit(self):
		self.Name='CMSPixel_ModuleTestGroup_Module_Chips_Chip_Summary_TestResult'
	...
	def PopulateResultData(self):
		...
		self.ResultData['KeyValueDictPairs'] = {
			'Total': {
				'Value':'{:1.0f}'.format(Total), 
				'Label':'Total'
			},
			'nDeadPixel': {
				'Value':'{:1.0f}'.format(nDeadPixel), 
				'Label':'Dead Pixels'
			},
			'nNoisy1Pixel': {
				'Value':'{:1.0f}'.format(nNoisy1Pixel), 
				'Label':'Noisy Pixels 1'
			},
			'nMaskDefect': {
				'Value':'{:1.0f}'.format(nMaskDefect), 
				'Label':'Mask Defects'
			},
			'nDeadBumps': {
				'Value':'{:1.0f}'.format(nDeadBumps), 
				'Label':'Dead Bumps'
			},
			'nDeadTrimbits': {
				'Value':'{:1.0f}'.format(nDeadTrimbits), 
				'Label':'Dead Trimbits'
			},
			'nAddressProblems': {
				'Value':'{:1.0f}'.format(nAddressProblems), 
				'Label':'Address Problems'
			},
			'nNoisy2Pixel': {
				'Value':'{:1.0f}'.format(nNoisy2Pixel), 
				'Label':'Noisy Pixels 2'
			},
			'nThrDefect': {
				'Value':'{:1.0f}'.format(nThrDefect), 
				'Label':'Trim Problems'
			},
			'nGainDefect': {
				'Value':'{:1.0f}'.format(nGainDefect), 
				'Label':'PH Gain defects'
			},
			'nPedDefect': {
				'Value':'{:1.0f}'.format(nPedDefect), 
				'Label':'PH Pedestal defects'
			},
			'nPar1Defect': {
				'Value':'{:1.0f}'.format(nPar1Defect), 
