...
class ProductionOverview(AbstractClasses.GeneralProductionOverview.GeneralProductionOverview):
    ...
    def GenerateOverview(self):
                    ...
                    if TestType == self.Attributes['Test']:
                        ...
                        for Chip in range(0, 16):
                            ...
                            if Sigma is not None and Mu is not None and float(Mu)>0 and Grade is not None:
                                ModuleGrade[Grade].append(float(Sigma) / float(Mu))
                                NROCs += 1
                            if Sigma is not None and Mu is not None and float(Mu)>0 and Grade is None:
                                ModuleGrade['3'].append(float(Sigma) / float(Mu))
                                NROCs += 1
        ...
        meanAll = round(h.GetMean()*1.e2,2)
        meanAB = round(hAB.GetMean()*1.e2,2)
        meanC = round(hC.GetMean()*1.e2,2)
        meanerrAll = round(h.GetMeanError()*1.e2,2)
        meanerrAB = round(hAB.GetMeanError()*1.e2,2)
        meanerrC = round(hC.GetMeanError()*1.e2,2)
        RMSAll = round(h.GetRMS()*1.e2,2)
        RMSAB = round(hAB.GetRMS()*1.e2,2)
        RMSC = round(hC.GetRMS()*1.e2,2)
        ...
        TitleText = "Mean: %.3f, RMS: %.4f, Gauss-fit sigma: %.4f"%(h.GetMean(), h.GetRMS(), GaussFitSigma)
