...
class GlobalDatabaseQuery():
    ...
    def GetFulltestPixelDefects(self, ModuleID, tempnominal = 'p17_1'):
        ...
        SQLQuery = '''
SELECT inventory_fullmodule.FULLMODULE_ID, tempnominal, ROC_POS, Total, nDeadPixel, nDeadBumps FROM inventory_fullmodule
...
WHERE inventory_fullmodule.FULLMODULE_ID = '{ModuleID}' AND tempnominal='{tempnominal}' AND test_fullmoduleanalysis.MACRO_VERSION = '{MacroVersion}' AND TYPE='FullQualification' AND STATUS='INSTOCK' ORDER BY ROC_POS
'''.format(MacroVersion=self.DBMacroVersion, tempnominal=tempnominal, ModuleID=ModuleID)
