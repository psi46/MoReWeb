...
class TestResult(AbstractClasses.GeneralTestResult.GeneralTestResult):
    ...
    def analyse_test_list(self, testList):
        ...
        while test:
            if 'fulltest' in test.testname.lower():
                ...
                QualificationAdded = True
            elif 'connectiontest' in test.testname.lower():
                print '\t-> appendConnectionTest'
                tests, test, index = self.appendConnectionTest(tests, test, index)
                QualificationAdded = True
            elif (
    ...
    def appendConnectionTest(self, tests, test, index):
        environment = test.environment
        key = 'Module%s_%s' % (test.testname, test.environment.name)
        nKeys = 1
        for item in tests:
        ...
        tests.append({
            ...
            'InitialAttributes': {
                ...
                'ModuleVersion': self.Attributes['ModuleVersion'],
                'ModuleType': self.Attributes['ModuleType'],
                'TestType': 'Connection_%s_%s' % (test.environment.name, nKeys),
                'TestTemperature': test.environment.temperature,
            },
            'DisplayOptions': {
                'Order': len(tests) + 1
