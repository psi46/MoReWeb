...
class SCurve_Fitting():
    ...
    def getVcal(self, mode, i):
        if mode == 0 and i < len(self.VcalVoltageCalibrationLowRange):
            return self.VcalVoltageCalibrationLowRange[i]
        if mode == 1 and i < len(self.VcalVoltageCalibrationHighRange):
            return self.VcalVoltageCalibrationHighRange[i]
        return -1

    def InitFit(self):
        #todo: do we really need 4 parameters or are 3 enough?
